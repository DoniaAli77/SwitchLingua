...
def generate_scenarios(pre_execute: dict) -> list:
    ...
    # shared lists
    domains: List[str] = shared["domain"]
    ...
    conversation_types: List[str] = char["conversation_type"]
    cs_functions: List[str] = shared["cs_function"]
    cs_types: List[str] = shared["cs_type"]

    use_tools: bool = bool(pre_execute.get("use_tools", shared.get("use_tools", False)))
    ...
    if "topic" in tasks:
        ...
        for base in base_scenarios:
            for lbl in topic_labels:
                ...
                s["topic"] = lbl
                s["task_constraints"] = {"topics": topic_labels}
