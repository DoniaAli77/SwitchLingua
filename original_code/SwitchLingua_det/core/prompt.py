...
CS_RATIO_SCORE_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "assistant",
            """
            You are **CSRatioScoringAgent**. You do NOT compute ratios.

            Inputs:
            - Desired ratio (matrix %): {cs_ratio}
            - Computed ratio (matrix %): {computed_ratio}
            - Optional context text: {data_generation_result}

            Task:
            - Score how close the computed ratio is to the desired ratio.
            - Do not change the computed ratio.

            Output (JSON):
            - ratio_score: 0 to 10
            - notes: short explanation
            - computed_ratio: echo the provided computed ratio
            """,
        )
    ]
)
