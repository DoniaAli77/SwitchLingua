...
RUN = 'experiments/outputs/multi_agent_bert/experiment_ahmed_polarity'
OUT = RUN + '/error_attribution'
os.makedirs(OUT, exist_ok=True)

# escalated 84 from the semantic_v1 th0.7 predictions
pr = json.load(open(glob.glob(RUN + '/full_agentic_th07_polarity/*predictions*.json')[0], encoding='utf-8'))
...
orch = build_orchestrator(tc, threshold=0.7, enable_deliberation=False,
                          ...
                          primary_classifier=primary, llm_client=llm, consensus_primary_weight=1.0, sentiment_agent_variant='lexical_polarity_contextual')
