"""Fair parallel-vs-sequential (same agents) — G2 selective gate, gpt-4o-mini.

Same isolation as the Design-C version, on the BEST Ahmed config: Lexical +
Polarity + Contextual voters + a non-voting selective IntentGate, gpt-4o-mini,
Ahmed precomputed primary, threshold 0.70, consensus w_primary 1.0. The single
change is ``task_config.sequential_chain`` (False vs True); it chains the three
VOTERS only (the gate stays independent and post-consensus). No training/generation.
"""
...
os.environ['SENTIMENT_AGENT_VARIANT'] = 'lexical_polarity_contextual_intent_gate'  # G2
...
MODEL = 'gpt-4o-mini'
VARIANT = 'lexical_polarity_contextual_intent_gate'
THRESH = 0.70
D = 'data/Sentiment/external/ahmed'
ALIGNED = D + '/ahmed_eesa_test_predictions_aligned.csv'
OUTDIR = 'experiments/outputs/multi_agent_bert/experiment_ahmed_parallel_vs_sequential_designG_4omini'
...
print('=== PARALLEL (sequential_chain=False) — G 4o-mini ===', flush=True)
par = run_mode(False)
print('=== SEQUENTIAL (sequential_chain=True) — G 4o-mini ===', flush=True)
seq = run_mode(True)
...
o = io.StringIO()
def P(s=''): o.write(s + '\n')
P('PARALLEL vs SEQUENTIAL — SAME AGENTS (Ahmed, Design G full gate, %s, threshold %.2f)' % (MODEL, THRESH))
P('Only difference: task_config.sequential_chain (chains the 3 voters; gate independent). 818 samples, %d escalated.' % mp['n_esc'])
...
open(OUTDIR + '/parallel_vs_sequential_designG_report.txt', 'w', encoding='utf-8').write(o.getvalue())
print('\n' + o.getvalue())
