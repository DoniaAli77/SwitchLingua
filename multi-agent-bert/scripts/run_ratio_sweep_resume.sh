...
for TAG in eesa50_gen025 eesa50_gen05 eesa50_gen10 eesa25_gen10; do
  ...
  "$PY" scripts/finetune_transformer_classifier.py \
    ...
    --seed 42 --fp16 --gradient_checkpointing --optim adafactor > "$OUT/finetune.log" 2>&1
  echo "  finetune exit=$? $(date +%H:%M:%S)"; sleep 15
  "$PY" evaluate_pipeline.py --dataset data/Sentiment/processed/eesa_sentiment_test.jsonl \
    --config src/config/default.yaml --active_task sentiment_classification \
    --pipeline_mode primary_only --mode full_pipeline --primary_model transformer \
    --transformer_checkpoint "$CK" --transformer_device cuda \
    --output_dir "$OUT/primary_only" --run_id "$TAG" > "$OUT/eval.log" 2>&1
