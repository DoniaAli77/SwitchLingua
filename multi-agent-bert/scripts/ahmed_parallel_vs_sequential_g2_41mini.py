"""Fair parallel-vs-sequential (same agents) — G2 selective gate, gpt-4.1-mini.

Same isolation as the Design-C version, on the BEST Ahmed config: Lexical +
Polarity + Contextual voters + a non-voting selective IntentGate, gpt-4.1-mini,
...
OUTDIR = 'experiments/outputs/multi_agent_bert/experiment_ahmed_parallel_vs_sequential_g2_41mini'
...
print('=== PARALLEL (sequential_chain=False) — G2 4.1-mini ===', flush=True)
par = run_mode(False)
print('=== SEQUENTIAL (sequential_chain=True) — G2 4.1-mini ===', flush=True)
seq = run_mode(True)
...
o = io.StringIO()
def P(s=''): o.write(s + '\n')
P('PARALLEL vs SEQUENTIAL — SAME AGENTS (Ahmed, G2 selective gate, %s, threshold %.2f)' % (MODEL, THRESH))
...
open(OUTDIR + '/parallel_vs_sequential_g2_report.txt', 'w', encoding='utf-8').write(o.getvalue())
print('\n' + o.getvalue())
