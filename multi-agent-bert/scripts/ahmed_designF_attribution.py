"""Per-agent capture for Design F (Lexical + Intent + Polarity + Contextual) on the
...
os.environ['SENTIMENT_AGENT_VARIANT'] = 'intent_polarity_contextual'
...
RUN = 'experiments/outputs/multi_agent_bert/experiment_ahmed_designF_intent_polarity_contextual'
...
ACTIVE = ["logic", "contextual", "polarity"]  # F: lexical abstains
...
print(f"Design F — {len(esc)} escalated", flush=True)
...
orch = build_orchestrator(tc, threshold=0.7, enable_deliberation=False,
                          ...
                          consensus_primary_weight=1.0, sentiment_agent_variant='intent_polarity_contextual')
...
n = len(rows)
# F active agents: Polarity(logic slot), Contextual, Intent(polarity slot). Lexical abstains.
COLS = {"polarity": "polarity_label", "contextual": "contextual_label", "intent": "intent_label"}
def acc(col):
    v = [r for r in rows if r[col] is not None]
    return round(sum(1 for r in v if r[col] == r['true_label']) / len(v), 4) if v else float('nan')
print("\n--- Design F per-agent accuracy (escalated) ---")
...
def pair(c1, c2):
    ...
    return round(sum(1 for r in v if r[c1] == r[c2]) / len(v), 4) if v else float('nan')
allag = sum(1 for r in rows if len({r[c] for c in COLS.values()}) == 1 and all(r[c] for c in COLS.values()))
print(f"\n   all-3-agree {allag}/{n} = {allag/n:.4f}")
