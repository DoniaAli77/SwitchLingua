"""EXPLORATORY sweep continuation: Topic-540 XLM-R + full agentic pipeline,
INCREMENTAL BANDS 0.40 <= c < 0.60 (460 rows) run ONCE.

Rows already decided at tau<=0.40 (386 of them) are NOT rerun; their saved
agent decisions are reused verbatim when tau=0.50 and tau=0.60 are assembled
offline. Because routing is monotonic in tau, this single run supplies both:
  tau=0.50 uses the 0.40-0.50 subset; tau=0.60 additionally uses 0.50-0.60.

Band membership was decided LABEL-BLIND from the saved primary confidence
column. --threshold 1.01 forces every pre-filtered row to escalate.
Exploratory only; tau=0.30 remains the predefined headline condition.

Instrumentation is byte-identical to the tau=0.30 runner (generated from it).
"""
...
OUT_DIR = ROOT / "experiments/outputs/multi_agent_bert/experiment_gen540_agentic_silver1163_tau0506_exploratory"
...
# ---------------------------------------------------------------- run it
ARGV = [
    "--dataset", "experiments/outputs/multi_agent_bert/experiment_gen540_agentic_silver1163_tau0506_exploratory/silver_band_040_060.jsonl",
