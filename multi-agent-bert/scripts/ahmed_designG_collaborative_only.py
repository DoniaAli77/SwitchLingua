"""Design G — sequential-COLLABORATIVE only (parallel + seq-independent already run).

Runs just the collaborative-framing sequential mode on Design G (full gate,
gpt-4o-mini), then combines with the existing parallel + sequential-independent
records from experiment_ahmed_parallel_vs_sequential_designG_4omini for the final
...
os.environ['SENTIMENT_AGENT_VARIANT'] = 'lexical_polarity_contextual_intent_gate'  # Design G
...
PRIOR = 'experiments/outputs/multi_agent_bert/experiment_ahmed_parallel_vs_sequential_designG_4omini/records.json'
OUTDIR = 'experiments/outputs/multi_agent_bert/experiment_ahmed_sequential_collaborative_designG'
...
print('=== Design G — sequential-collaborative ===', flush=True)
...
o = io.StringIO()
def P(s=''): o.write(s + '\n')
P('SEQUENTIAL FRAMING TEST — DESIGN G (full gate), Ahmed, %s, threshold %.2f' % (MODEL, THRESH))
...
open(OUTDIR + '/collaborative_designG_report.txt', 'w', encoding='utf-8').write(o.getvalue())
