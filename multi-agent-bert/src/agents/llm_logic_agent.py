"""LLM-based logical reasoning specialist agent for full_agentic pipeline mode.

Delegates logic-based classification to an LLM via a pluggable ``LLMClient``.
Unlike the regex-based :class:`~src.agents.logic_agent.LogicAgent`, this agent
instructs the model to reason about structural patterns, relational cues, and
domain-specific co-occurrence structures in both Arabic and English.

The agent:

1. Builds a structured prompt focused on logical/rule-based reasoning.
...
_PARSE_FAIL_NOTE = "LLMLogicAgent: response parse failed; uniform fallback applied."
_INVALID_LABEL_NOTE = "LLMLogicAgent: LLM returned an invalid label; uniform fallback applied."


class LLMLogicParseError(ValueError):
...
class LLMLogicAgent(BaseAgent[PipelineState]):
    """LLM-backed logical reasoning classifier for the full_agentic pipeline mode.
    ...
    def __init__(
        ...
        super().__init__(name=name or "LLMLogicAgent", logger=logger)
    ...
    def validate_before(self, state: PipelineState) -> None:
        if not state.input_text or not state.input_text.strip():
            raise ValueError("LLMLogicAgent: state.input_text is empty or blank.")
        if not state.task_config.labels:
            raise ValueError("LLMLogicAgent: state.task_config.labels is empty.")
    ...
    def run(self, state: PipelineState) -> PipelineState:
        ...
        except LLMLogicParseError as exc:
        ...
        state.append_history(
            component=self.name,
            summary=f"Logic label '{label}' (confidence={confidence:.3f}). {reasoning}",
            outputs={
                "label": label,
                "confidence": confidence,
    ...
    def _parse_response(self, raw: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise LLMLogicParseError(f"JSON decode failed: {exc}") from exc

        if not isinstance(data, dict):
            raise LLMLogicParseError(f"Expected a JSON object, got {type(data).__name__}")

        missing = _REQUIRED_KEYS - data.keys()
        if missing:
            raise LLMLogicParseError(f"Missing required keys: {sorted(missing)}")

        if not isinstance(data["confidence"], (int, float)):
            raise LLMLogicParseError(
