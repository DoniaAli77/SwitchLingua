...
async def main():
    parser = argparse.ArgumentParser(description="Run full SwitchLingua pipeline.")
    parser.add_argument("--config", default=None, help="Path to config yaml.")
    parser.add_argument(
        "--max-scenarios",
        type=int,
        default=0,
        help="Optional cap on number of shuffled scenarios (0 = all).",
    )
    args = parser.parse_args()

    default_config = str((Path(__file__).resolve().parent.parent / "config" / "config2.yaml"))
    config_path = args.config or os.getenv("SWITCHLINGUA_CONFIG_PATH") or default_config
    ...
    if args.max_scenarios and args.max_scenarios > 0:
        ...
        print("max_scenarios applied:", len(scenarios))
